...
const PostServiceForm = () => {
  ...
  const [packageLimits, setPackageLimits] = useState({ packageId: '', photoUploads: 0, videoUploads: 0 });
      ...
      try {
        ...
        if (userData.accountType === 'seller' && userData.sellerPackage) {
          setPackageLimits({
            ...
            videoUploads: userData.sellerPackage.videoUploads || 0,
          });
        } else {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newVideos = Array.from(files).filter(f => f.type.startsWith('video/')).length;
    ...
    if (currentVideos + newVideos > packageLimits.videoUploads) {
      toast.error(`Your ${packageLimits.packageId} package allows only ${packageLimits.videoUploads} video(s)`);
      return;
    }
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      const video = uploadedFiles.find(file => file.type === 'video')?.preview;
      ...
      const response = await axios.post('https://themabinti-main.onrender.com/api/services', payload, {
        headers: {
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-4">
                ...
                {/* Show upload button if limits not reached */}
                {(uploadedFiles.filter(f => f.type === 'image').length < packageLimits.photoUploads || 
                   uploadedFiles.filter(f => f.type === 'video').length < packageLimits.videoUploads) && (
                  <div className="border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center h-32 relative">
                    ...
                    <div className="text-center">
                      ...
                      <span className="text-sm text-gray-500 mt-1 block">Upload</span>
                    </div>
                  </div>
                )}
              </div>
